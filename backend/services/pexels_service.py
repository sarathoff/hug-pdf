...
class PexelsService:
    ...
    def search_images_sync(self, query: str, per_page: int = 15, page: int = 1) -> Optional[Dict]:
        """
        Search for images on Pexels (Synchronous)
        """
    ...
    async def search_images(self, query: str, per_page: int = 15, page: int = 1) -> Optional[Dict]:
        """
        Search for images on Pexels (Async wrapper)
        """
        return await asyncio.to_thread(self.search_images_sync, query, per_page, page)

    def get_curated_images_sync(self, per_page: int = 15, page: int = 1) -> Optional[Dict]:
        """
        Get curated images from Pexels (Synchronous)
    ...
    async def get_curated_images(self, per_page: int = 15, page: int = 1) -> Optional[Dict]:
        """
        Get curated images from Pexels (Async wrapper)
        """
        return await asyncio.to_thread(self.get_curated_images_sync, per_page, page)
