        ...
        try:
            ...
            if 'localhost' in url or '127.0.0.1' in url:
                if '/temp-images/' in url:
                    ...
                    base_temp_dir = (PathLib(__file__).parent.parent / "temp_uploads").resolve()
                    source_path = (base_temp_dir / filename).resolve()

                    # Security Check: Prevent path traversal
                    try:
                        if not source_path.is_relative_to(base_temp_dir):
                            logger.error(
                            f"SECURITY: Path traversal attempt in _download_image: {filename}"
                        )
                            return None
                    except Exception:
                        return None
