...
class PDFService:
    @staticmethod
    def _generate_pdf_sync(latex_content: str) -> bytes:
        """Synchronous implementation of PDF generation"""
        logger.info("Attempting PDF generation from LaTeX (Sync)")
    ...
    async def generate_pdf(latex_content: str) -> bytes:
        """Convert LaTeX to PDF using pdflatex (Non-blocking)"""
        return await asyncio.to_thread(PDFService._generate_pdf_sync, latex_content)
