            ...
            if 'localhost' in url or '127.0.0.1' in url:
                if '/temp-images/' in url:
                    ...
                    if not source_path.is_relative_to(uploads_dir):
                        logger.warning(f"Security Alert: Path traversal attempt in PDF generation blocked. File: {filename}")
                        return None

                    if source_path.exists():
                        # Copy to temp directory
                        # Also validate dest_path to ensure we don't write outside temp_dir
                        dest_path = (temp_dir / filename).resolve()

                        if not dest_path.is_relative_to(temp_dir):
                            logger.warning(f"Security Alert: Destination path traversal blocked. File: {filename}")
                            return None
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                if pdf_file.exists():
                    ...
                    logger.error("pdflatex failed to create PDF.")
