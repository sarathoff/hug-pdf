        ...
        try:
            ...
            if 'localhost' in url or '127.0.0.1' in url:
                if '/temp-images/' in url:
                    ...
                    filename = os.path.basename(filename) # Prevent traversal in filename

                    # Construct and verify path to uploaded file
                    from pathlib import Path as PathLib
                    temp_uploads_dir = (PathLib(__file__).parent.parent / "temp_uploads").resolve()
                    source_path = (temp_uploads_dir / filename).resolve()

                    # Security check: Ensure we are still inside temp_uploads
                    if not source_path.is_relative_to(temp_uploads_dir):
                        logger.warning(f"LFI Attempt blocked: {url} -> {source_path}")
                        return None
