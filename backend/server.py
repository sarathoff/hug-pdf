...
# User cache to reduce DB calls (TTL: 60 seconds)
...
async def get_current_user(authorization: Optional[str] = Header(None)) -> Optional[dict]:
    ...
    if user_id in user_cache:
        logging.info(f"DEBUG: User {user_id} found in cache")
...
async def download_pdf(
    ...
    try:
        ...
        if current_user:
            # Fetch fresh credits to avoid using stale cached data
            try:
                if supabase:
                    fresh_data = supabase.table("users").select("credits").eq("user_id", current_user['user_id']).execute()
                    if fresh_data.data:
                        current_user['credits'] = fresh_data.data[0]['credits']
            except Exception as e:
                logger.warning(f"Failed to fetch fresh credits: {e}")
            ...
            if supabase_admin:
                ...
                # Update local object (and cache)
                current_user['credits'] -= 1

                logger.info(f"Deducted 1 PDF credit from user {current_user['user_id']}. Remaining: {current_user['credits']}")
...
async def payment_success(
    ...
    try:
        ...
        # Invalidate cache so user sees new credits/plan immediately
        if user_id in user_cache:
