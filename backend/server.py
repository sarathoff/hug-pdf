...
# User cache to reduce database hits (1 minute TTL)
...
async def get_current_user(authorization: Optional[str] = Header(None)) -> Optional[dict]:
    ...
    # Fetch from Supabase (non-blocking)
    try:
        ...
        logging.error(f"Supabase fetch error for user {user_id}: {e}")
        return None
    ...
    # Cache the result
    user_cache[user_id] = response.data[0]
...
async def download_pdf(
    ...
    try:
        ...
        if current_user:
            # Fetch fresh credits to ensure accuracy before deduction (bypass cache)
            # Use supabase client to fetch fresh data
            if supabase:
                try:
                    fresh_resp = await asyncio.to_thread(
                        lambda: supabase.table("users").select("credits").eq("user_id", current_user['user_id']).execute()
                    )
                    if fresh_resp.data:
                        current_user['credits'] = fresh_resp.data[0]['credits']
                except Exception as e:
                    logger.error(f"Failed to refresh credits: {e}")
