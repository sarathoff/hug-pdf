...
def _get_user_from_db_sync(user_id: str, email: Optional[str] = None) -> Optional[dict]:
    """Synchronous function to fetch user from DB"""
    if not supabase:
...
async def get_current_user(authorization: Optional[str] = Header(None)) -> Optional[dict]:
    ...
    # Run token verification in thread to avoid blocking CPU
    payload = await asyncio.to_thread(auth_service.verify_token, token)
    ...
    # Run DB fetch in thread to avoid blocking event loop
    return await asyncio.to_thread(_get_user_from_db_sync, payload['user_id'], payload.get('email'))
