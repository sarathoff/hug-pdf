...
async def upload_image(file: UploadFile = File(...), current_user: dict = Depends(get_current_user)):
    try:
        ...
        # Check MIME type (trusting header, but better than nothing)
...
async def serve_temp_image(filename: str):
    ...
    # 1. Base name only (removes paths)
    safe_filename = os.path.basename(filename)

    # 2. Resolve path and ensure it's in temp_uploads
    temp_dir = (ROOT_DIR / "temp_uploads").resolve()
    filepath = (temp_dir / safe_filename).resolve()
