...
async def serve_temp_image(filename: str):
    ...
    # Resolve the full path including the filename
    filepath = (base_dir / filename).resolve()

    # Check if the resolved path is within the base_dir
    if not filepath.is_relative_to(base_dir) or not filepath.exists():
        logger.warning(f"SECURITY: Attempted path traversal or file not found: {filename} -> {filepath}")
