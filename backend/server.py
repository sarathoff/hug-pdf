...
def _get_current_user_sync(token: str, supabase_client: Client, supabase_admin_client: Client) -> Optional[dict]:
    """Synchronous logic for getting current user to be run in a thread"""
    ...
    response = supabase_client.table("users").select("*").eq("user_id", payload['user_id']).execute()
    ...
    if not response.data:
        ...
        try:
            ...
            res = supabase_admin_client.table("users").insert(new_user).execute()
            if res.data:
            ...
            if '23505' in error_str or 'duplicate key' in error_str.lower():
                ...
                retry_response = supabase_admin_client.table("users").select("*").eq("user_id", payload['user_id']).execute()
...
async def get_current_user(authorization: Optional[str] = Header(None)) -> Optional[dict]:
    ...
    # Run blocking DB/Auth operations in a separate thread to avoid blocking the event loop
    return await asyncio.to_thread(_get_current_user_sync, token, supabase, supabase_admin)
