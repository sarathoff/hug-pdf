...
async def payment_success(
    ...
    if not session_id:
        logger.warning(f"Payment success attempted without session_id for user {user_id}")
        raise HTTPException(status_code=400, detail="Payment session ID required")

    # Allow test sessions for local development but REQUIRE explicitly enabled test mode
    if session_id.startswith('test_session_'):
        if os.environ.get('PAYMENT_TEST_MODE') != 'true':
            logger.warning(f"Test session {session_id} rejected (PAYMENT_TEST_MODE not enabled)")
            raise HTTPException(status_code=403, detail="Test sessions not allowed in this environment")
