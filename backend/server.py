...
# Initialize User Cache (TTL: 60 seconds)
# Stores user_id -> user_dict mapping to reduce DB calls
...
async def download_pdf(
    ...
    try:
        ...
        if current_user:
            # Force fetch fresh credits to ensure consistency (bypass cache)
            user_id = current_user['user_id']
            fresh_credits = current_user['credits']  # Fallback

            if supabase_admin:
                resp = supabase_admin.table("users").select("credits").eq("user_id", user_id).execute()
                if resp.data:
                    fresh_credits = resp.data[0]['credits']
            ...
            if supabase_admin:
                supabase_admin.table("users").update({
                    'credits': fresh_credits - 1,
                    'updated_at': datetime.now(timezone.utc).isoformat()
                }).eq('user_id', user_id).execute()

                logger.info(f"Deducted 1 PDF credit from user {user_id}. Remaining: {fresh_credits - 1}")

                # Invalidate cache so next request sees updated credits
                if user_id in user_cache:
...
async def payment_success(
    ...
    try:
        ...
        # Invalidate cache so user sees new credits immediately
        if user_id in user_cache:
            del user_cache[user_id]
