...
# Initialize caches
# Cache user data for 60 seconds to reduce DB calls on frequent requests
...
async def download_pdf(
    ...
    try:
        ...
        if current_user:
            # Race condition fix: fetch fresh credits from DB before deducting
            if supabase:
                response = supabase.table("users").select("credits").eq("user_id", current_user['user_id']).execute()
                if response.data:
                    current_credits = response.data[0]['credits']
                else:
                    current_credits = current_user['credits'] # Fallback to cached
            else:
            ...
            if supabase:
                ...
                # Invalidate cache so next request gets updated credits
