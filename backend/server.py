...
async def get_current_user(authorization: Optional[str] = Header(None)) -> Optional[dict]:
    ...
    # OPTIMIZATION: Use asyncio.to_thread to prevent blocking the event loop with synchronous Supabase calls
    response = await asyncio.to_thread(lambda: supabase.table("users").select("*").eq("user_id", payload['user_id']).execute())
    ...
    if not response.data:
        ...
        try:
            ...
            res = await asyncio.to_thread(lambda: supabase_admin.table("users").insert(new_user).execute())
            if res.data:
            ...
            if '23505' in error_str or 'duplicate key' in error_str.lower():
                ...
                retry_response = await asyncio.to_thread(lambda: supabase_admin.table("users").select("*").eq("user_id", payload['user_id']).execute())
