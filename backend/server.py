...
async def preview_pdf(request: DownloadPDFRequest):
    ...
    try:
        ...
        if request.latex_content and pdf_service.is_latex_available():
            pdf_bytes = await pdf_service.generate_pdf(request.latex_content)
        # Fallback to HTML if LaTeX unavailable or not provided
        elif request.html_content:
            # If LaTeX was requested but unavailable, log a warning
            if request.latex_content:
                logger.warning("LaTeX content provided but pdflatex not available. Falling back to HTML generation.")

            pdf_bytes = await pdf_service.generate_pdf_from_html(request.html_content)
        # If only LaTeX provided but unavailable
        elif request.latex_content:
             raise HTTPException(
                status_code=503,
                detail="PDF generation unavailable: pdflatex not installed and no HTML content provided for fallback."
            )
        else:
...
async def download_pdf(
    ...
    try:
        ...
        # Use LaTeX if available and requested
        if request.latex_content and pdf_service.is_latex_available():
            pdf_bytes = await pdf_service.generate_pdf(request.latex_content)
        # Fallback to HTML if LaTeX unavailable or not provided
        elif request.html_content:
