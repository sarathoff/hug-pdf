...
# Cache for user data: 1000 users, 60 seconds TTL
...
async def download_pdf(
    ...
    try:
        ...
        if current_user:
            # Fetch fresh data from DB to ensure credits are up-to-date and avoid race conditions
            # or using stale cache data
            fresh_user_resp = supabase.table("users").select("credits").eq("user_id", current_user['user_id']).execute()
            if not fresh_user_resp.data:
                raise HTTPException(status_code=404, detail="User not found")

            current_credits = fresh_user_resp.data[0]['credits']
            ...
            if supabase_admin:
                ...
                if current_user['user_id'] in user_cache:
                    user_cache[current_user['user_id']]['credits'] = new_credits
