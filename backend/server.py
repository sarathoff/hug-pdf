...
# User cache (60s TTL, max 1000 items)
...
# Helper function for blocking DB calls
def _fetch_user_sync(user_id: str, email: Optional[str] = None) -> Optional[dict]:
    """Blocking helper to fetch or create user in Supabase"""
    if not supabase:
    ...
    if not response.data:
        ...
        try:
            ...
            if '23505' in error_str or 'duplicate key' in error_str.lower():
                ...
                retry_response = supabase_admin.table("users").select("*").eq("user_id", user_id).execute()
...
async def get_current_user(authorization: Optional[str] = Header(None)) -> Optional[dict]:
    ...
    # Cache miss: fetch from DB (in thread pool to avoid blocking)
    try:
        user_data = await asyncio.to_thread(_fetch_user_sync, user_id, payload.get('email'))
        if user_data:
...
async def payment_success(
    ...
    # ... (verification logic omitted for brevity in thought, but included in full file write) ...
    ...
    if verification_id:
        ...
        if current_user:
            ...
            logger.error("Security Alert: Unauthenticated payment attempt without verification ID")
            raise HTTPException(
                status_code=401,
