                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <span tabIndex={0} className="inline-flex cursor-not-allowed">
                                        <Button
                                            size="icon"
                                            className={`h-10 w-10 flex-shrink-0 transition-all duration-200 ${input.trim() ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-200 text-gray-400'}`}
                                            onClick={handleSendMessage}
                                            disabled={!input.trim() || loading || !sessionId}
                                            aria-label="Send message"
                                            style={{ pointerEvents: (!input.trim() || loading || !sessionId) ? 'none' : 'auto' }}
                                        >
